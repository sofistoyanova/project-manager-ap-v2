...
const Task = (props) => {
  const handleFormSubmit = async (event) => {
    ...
    let updateTaskResponse = await patchMethod(
      `/api/projects/task/update/${props.taskId}`,
      formData
    );
  ...
  const updateTask = async (event) => {
    ...
    const updateTaskForm = container.querySelector(".updateTaskForm");
    updateTaskForm.classList.toggle("displayBlock");
  };

  const deleteTask = async () => {
    let deleteTaskResponse = await patchMethod(
      `/api/projects/task/delete/${props.taskId}`
    );
      ...
      <div className="taskDetails">
        <div className="checkImageContainer">
          <img
            ...
            src={props.taskStatus ? checkImage : uncheckImage}
          />
        </div>
