        ...
        if(isFormDataValid.isValid) {
            try {
                ...
                if(addProjectResponse.status === 200) {
                    this.setState({addProjectStatusMessage: addProjectResponse.message})
                } else {
                    this.setState({addProjectStatusMessage: addProjectResponse.message})
                }
            } catch(err) {
                this.setState({addProjectStatusMessage: 'There was an error please try again later!'})
            }
        } else {
            ...
            <div className="CreateProjectformContainer">
                ...
                <div className="CreateProjectformInnerContainer">
                    <form className="loginForm" onSubmit={this.handleFormSubmit}>
                        ...
                        <input className="loginInput" type="text" name="projectName" placeholder="Project Name" required />
                        <label className="loginLabel">Project description:</label>
                        <textarea className="loginInput" placeholder="*optional" name="projectDescription" rows="4"></textarea>
                        <input className="createProjectButton" type="submit" value="+ Add Project" />
                    </form>
                </div>
            </div>
        )
